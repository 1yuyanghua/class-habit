...
export default function StudentsPage() {
  ...
  const filtered = students.filter((s) => {
    const matchSearch = !search || s.name.includes(search) || s.studentNo.includes(search) || s.parentPhone.includes(search);
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <div className="relative flex-1">
            ...
            <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="搜索姓名、学号、电话号码..." className="w-full pl-10 pr-4 py-2 rounded-lg border border-slate-200 bg-white text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500" />
          </div>
              ...
              {filtered.map((stu) => {
                ...
                return (
                  <tr key={stu.id} onClick={() => { setSelectedStudent(stu); setEditMode(false); }} className="border-b border-slate-100 hover:bg-indigo-50/50 cursor-pointer group">
                    ...
                    <td className="px-4 py-3 text-sm text-slate-600"><div>{stu.parentName}</div><a href={`tel:${stu.parentPhone}`} className="text-xs text-indigo-600 hover:text-indigo-800 flex items-center gap-1"><Phone className="w-3 h-3" />{stu.parentPhone}</a></td>
            ...
            return (
              <div key={stu.id} className="relative bg-white rounded-xl border border-slate-200 p-4">
                ...
                <button onClick={() => { setSelectedStudent(stu); setEditMode(false); }} className="w-full text-left pr-8">
                  ...
                  <div className="mt-2 pt-2 border-t border-slate-100 flex items-center justify-between text-xs text-slate-500"><span className={`px-2 py-0.5 rounded-full ${stu.boarding === "住宿生" ? "bg-blue-50 text-blue-600" : "bg-orange-50 text-orange-600"}`}>{stu.boarding}</span><span>{stu.parentName} · <a href={`tel:${stu.parentPhone}`} className="text-indigo-600 hover:text-indigo-800">{stu.parentPhone}</a></span></div>
                </button>
              </div>
            );
          })}
                ...
                <>
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      ...
                      <div className="bg-green-50 rounded-lg p-3"><p className="text-xs text-slate-400">联系电话</p><a href={`tel:${selectedStudent.parentPhone}`} className="text-sm text-indigo-600 hover:text-indigo-800 mt-0.5 flex items-center gap-1">{selectedStudent.parentPhone || "—"} <Phone className="w-3 h-3" /></a></div>
                    </div>
                  </div>
